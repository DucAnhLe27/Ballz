        ...
        if (other.gameObject.tag == "Ball" && (ballControl.currentBallState == ballState.FIRE || ballControl.currentBallState == ballState.ENDSHOT))
        {
            ...
            switch (type)
            {
                case PowerupType.VERTICAL:
                    hit.GetComponent<LineRenderer>().SetPosition(0, new Vector3(transform.position.x, -5, 0));
                    hit.GetComponent<LineRenderer>().SetPosition(1, new Vector3(transform.position.x, 5, 0));
                    hit.SetActive(true);
                    Invoke("EndHit", 0.05f);
                    break;
